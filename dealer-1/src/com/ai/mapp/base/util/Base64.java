...
public class Base64 {
	...
	/**
	 * * Encode an array of bytes as Base64. * It will be broken into lines if
	 * the line length is not 0. If broken into lines, * the last line is not
	 * terminated with a line separator. * * param ba The byte array to encode.
	 * */
	public String encode(byte[] bin, int str, int len) {
		...
		// output length
		StringBuffer sb;
		// string buffer for output(must be local for recursion to work).
		int lp;
		// line position(must be local for recursion to work).
		int el;
		// even multiple of 3 length
		int ll;
		// leftover length
		...
		if (ll == 1) {
			sb.append(
					...
					.append("==");
			// Use recursion so escaping logic is not repeated, replacing last 2
			// chars with "==".
		} else if (ll == 2) {
			sb.append(
					encode(new byte[] { bin[el + str], bin[el + str + 1], 0 })
							.substring(0, 3)).append("=");
			// Use recursion so escaping logic is not repeated, replacing last
			// char and "=".
		}
		if (ol != sb.length()) {
	...
	public byte[] decode(String b64, int str, int len) {
		...
		// target byte array
		int dc;
		// decode cycle (within sequence of 4 input chars).
		int rv;
		// reconstituted value
		int ol;
		// output length
		...
		// padding count
		ba = new byte[(len / 4) * 3];
		// create array to largest possible size.
		dc = 0;
		rv = 0;
		ol = 0;
		pc = 0;
	...
	/**
	 * * Set maximum line length for encoded lines. * Ignored by decode. * @param
	 * len Length of each line. 0 means no newlines inserted. Must be a multiple
	 * of 4.
	 * */
	 ...
	 * any chars except 'A'-'Z', 'a'-'z', '0'-'9', '+' and '/'. * @param linsep
	...
	// translation array for encoding
	static private final int[] DECODE = new int[256];
	// translation array for decoding
	static private final int IGNORE = -1;
	// flag for values to ignore when decoding
	static private final int PAD = -2;
	// flag value for padding value when decoding
