...
import com.ai.mapp.base.entity.Page;
import com.ai.mapp.base.error.RollbackException;
import com.ai.mapp.base.util.PropertyFilter;
import com.ai.mapp.base.util.ReflectionUtils;
import com.ai.mapp.base.util.PropertyFilter.MatchType;
 ...
 * @param <PK> 主键类型
 */
public abstract class HibernateDao<T, PK extends Serializable> extends HibernateNativeDao<T, PK> {
	/**
	 * 用于Dao层子类使用的构造函数.
...
//
	...
	public Page<T> findPage(final Page<T> page, Properties alias,final Criterion... criterions) {
		Assert.notNull(page, "page不能为空");

		Criteria c = createCriteria(criterions);
		
		//增加别名
		for(Object key:alias.keySet()){
			String alia = alias.getProperty(key.toString());
			c.createAlias(alia, key.toString());
		}		
	...
	protected Query setPageParameter(final Query q, final Page<T> page) {
		...
		if(page.isIfOffset())
		{
			...
			q.setMaxResults(page.getOffset());
		}
		else
		{
	...
	protected Criteria setPageParameter(final Criteria c, final Page<T> page) {
		...
		if(page.isAutoLimit())
		{
			...
			if(page.isIfOffset())
			{
				...
				c.setMaxResults(page.getOffset());
			}
			else
			{
	...
	protected int countCriteria(final Criteria criteria){
...
//		System.out.println("==========================="+countCriteria(c)+"============================");
		...
		// 执行Count查询
		Integer t =  (Integer) c.setProjection(Projections.rowCount()).uniqueResult();
		int totalCount = t == null?0:t.intValue();
...
//		int totalCount = 0;
	...
	public Criteria createCriteria(T t) throws Exception{
		Criteria criteria = getSession().createCriteria(entityClass);
		return createCriteria(criteria, t);
	}
	
	/**
	 * 按Criteria分页查询.
	 * @param page 分页参数.
	 * @param Criteria Hibernate条件查询对象
	 * @return 分页查询结果.附带结果列表及所有查询时的参数.
	 * @author mler
	 * @since 2010-12-09
	 * 
	 ...
	 * @throws Exception 
	 */
	abstract public Criteria createCriteria(Criteria c,T t) throws Exception;
	 ...
	 * 2012-12-15
	 */
	public int count(T t) throws RollbackException {
	 ...
	 * 2012-12-15
	 */
	public Collection<T> list(T t,int start,int limit,final boolean... ifOffset) throws RollbackException
	{
		try{
			...
			page = c == null ? findPage(page) : findPage(page, c);
	...
	public Collection<T> listAll(T t) throws RollbackException
	{
		try{
