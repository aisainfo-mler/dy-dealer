...
@Scope("prototype")
public class HW0001Action extends
		AbstractYDBaseActionHandler<HW0001Request, HW0001Response> {
	...
	protected void doAction() throws BusinessException, SystemException,
		...
		if (request.getCodes() != null && request.getCodes().isEmpty() == false) {
			...
			products = productService.listProducts(condition,
					request.getPage(), request.getSize());
		}

		/** 设置子plan **/
		Map<String, Product> planMap = gePlanMap(products);
		List<HW0001Response.Product> hw_products = handlerProduct(products,
				planMap);
		response.setProducts(hw_products);
	}

	private Map<String, Product> gePlanMap(Collection<Product> products)
			throws Exception {
		...
		for (Product p : products) {
			...
			List<Plan2ProductMapping> ppm_list = mapper.readValue(
					p.getPlancodes(),
					new TypeReference<List<Plan2ProductMapping>>() {
					});
		...
		Collection<Product> plan_list = productService
	...
	private List<HW0001Response.Product> handlerProduct(
			Collection<Product> products, Map<String, Product> planMap)
			throws Exception {
		if (products == null || products.isEmpty())
			return null;

		List<HW0001Response.Product> hw_products = new ArrayList<HW0001Response.Product>(
				0);
		response.setProducts(hw_products);
		...
		for (Product p : products) {
			...
			hw_p.setPrice(p.getPrice() == null ? BigDecimal.ZERO : BigDecimal
					.valueOf(p.getPrice()));
	 ...
	 * @throws Exception
	 */
	private List<HW0001Response.ProductSpec> handlerProductSpecList(Product p)
			throws Exception {
		...
		if (productSpecMapping == null
				...
				|| productSpecMapping.getProductSpecs().isEmpty())
			return null;

		List<ProductSpecMapping.ProductSpec> productSpecList = productSpecMapping
				.getProductSpecs();
		List<HW0001Response.ProductSpec> hw_productSpecList = new ArrayList<HW0001Response.ProductSpec>(
				0);
		...
		for (ProductSpecMapping.ProductSpec ps : productSpecList) {
			...
			if (ps.getIdentifiers() != null
				...
				for (ProductSpecMapping.Identifier idf : ps.getIdentifiers()) {
					...
					hw_idf.setComponentPriceId(idf.getComponentPriceId());
					hw_idf.setName(idf.getValue());
					hw_idf.setPrice(StringUtils.isBlank(idf.getComponentPrice()) ? BigDecimal.ZERO
							: new BigDecimal(idf.getComponentPrice()));
			...
			hw_ps.setPrice(StringUtils.isBlank(ps.getCompositePrice()) ? BigDecimal.ZERO
					: new BigDecimal(ps.getCompositePrice()));
			...
			if (ps.getServiceSpecList() != null
					&& ps.getServiceSpecList().isEmpty() == false) {
				List<HW0001Response.ServiceSpec> hw_serviceSpecList = new ArrayList<HW0001Response.ServiceSpec>(
						0);
				hw_ps.setServiceSpecList(hw_serviceSpecList);

				for (ProductSpecMapping.ServiceSpec ss : ps
						.getServiceSpecList()) {
					...
					if (ss.getIdentifiers() != null
						...
						for (ProductSpecMapping.Identifier idf : ss
							...
							HW0001Response.Identifier hw_idf = new HW0001Response.Identifier();
							hw_idf.setCode(idf.getId());
							hw_idf.setComponentPriceId(idf
									.getComponentPriceId());
							hw_idf.setName(idf.getValue());
							hw_idf.setPrice(StringUtils.isBlank(idf
									.getComponentPrice()) ? BigDecimal.ZERO
									: new BigDecimal(idf.getComponentPrice()));
							hw_idf.setSelectable(idf.getIsCustomerSelectable());
			...
			if (ps.getResourceSpecList() != null
					&& ps.getResourceSpecList().isEmpty() == false) {
				List<HW0001Response.ResourceSpec> hw_resourceSpecList = new ArrayList<HW0001Response.ResourceSpec>(
						0);
				hw_ps.setResourceSpecList(hw_resourceSpecList);

				for (ProductSpecMapping.ResourceSpec rs : ps
						.getResourceSpecList()) {
					...
					hw_rs.setPrice(StringUtils.isBlank(rs.getComponentPrice()) ? BigDecimal.ZERO
							: new BigDecimal(rs.getComponentPrice()));
				}
			}
		}
	 ...
	 * @throws Exception
	 */
	private List<HW0001Response.PlanSpec> handlerPlanSpecList(Product p)
			throws Exception {
		if (StringUtils.isBlank(p.getPlanSpecList()))
			return null;

		PlanSpecMapping planSpecMapping = DealerDataService.mapper.readValue(
				p.getPlanSpecList(), PlanSpecMapping.class);
		/** 设置planSpec **/
		if (planSpecMapping == null || planSpecMapping.getPlanSpecs() == null
				|| planSpecMapping.getPlanSpecs().isEmpty())
			return null;

		List<PlanSpecMapping.PlanSpec> planSpecList = planSpecMapping
				.getPlanSpecs();
		List<HW0001Response.PlanSpec> hw_planSpecList = new ArrayList<HW0001Response.PlanSpec>(
				0);
		...
		for (PlanSpecMapping.PlanSpec ps : planSpecList) {
			...
			hw_ps.setPrice(StringUtils.isBlank(ps.getComponentPrice()) ? BigDecimal.ZERO
					: new BigDecimal(ps.getComponentPrice()));
			hw_ps.setName(ps.getName());
			hw_ps.setType(ps.getType());

			/** 设置PlanSpec下面的ServiceSpec **/
			if (ps.getPlanSpecCharList() != null
					&& ps.getPlanSpecCharList().isEmpty() == false) {
				List<HW0001Response.PlanSpecChar> hw_planSpecCharList = new ArrayList<HW0001Response.PlanSpecChar>(
						0);
				hw_ps.setPlanSpecCharList(hw_planSpecCharList);

				for (PlanSpecMapping.PlanSpecCharacteristic psc : ps
						.getPlanSpecCharList()) {
