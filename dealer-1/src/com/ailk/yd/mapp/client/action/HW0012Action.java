...
@Scope("prototype")
public class HW0012Action extends
		AbstractYDBaseActionHandler<HW0012Request, IBody> {
	...
	protected Map<String, String> getResource(HW0010Request hw0010_req)
			throws Exception {
		if (hw0010_req.getCafInfos() == null
				|| hw0010_req.getCafInfos().isEmpty())
	...
	public YD0010Request convertByHW0010(HW0010Request hw0010_req)
			throws Exception {

		/**
		 * 出错过的字段 "nationality": "Andorra"-->'HR' "salutation": "Z001",
		 * -->'0001' "anniversaryDate": ""-->'2014-09-08',
		 */
		...
		// xuzhou start 1代表.ProductSpec，2.代表serviceSpac，3.代表resourceSpec
		Map<String, Map<String, List<com.ailk.yd.mapp.client.model.HW0010Request.Identifier>>> identifierMap = caf
		...
		req.setReferenceNumber(hw_caf_order.getOrn() == null ? ""
				: hw_caf_order.getOrn());
		req.setCustomerId(hw_customer.getCustomerId() == null ? ""
				: hw_customer.getCustomerId());
		req.setOrderType("CREATE");
		req.setCircleId("TC");
		req.setAppointmentDateTimeFrom(hw_caf_order
				.getAppointmentDateTimeFrom() == null ? "" : hw_caf_order
				.getAppointmentDateTimeFrom());
		req.setAppointmentDateTimeTo(hw_caf_order.getAppointmentDateTimeTo() == null ? ""
				: hw_caf_order.getAppointmentDateTimeTo());
		// 测试数据

		hw_caf_order.setChannel("10");
		req.setChannel(hw_caf_order.getChannel() == null ? "" : hw_caf_order
				.getChannel());
		req.setDeliveryMode(hw_caf_order.getDeliveryMode() == null ? ""
				: hw_caf_order.getDeliveryMode());

		/******** customerDetail 设置 ***********/
		req.setCustomerDetails(new Customer());
		req.getCustomerDetails().setProspectId(
				hw_customer.getProspectId() == null ? "" : hw_customer
						.getProspectId());
		req.getCustomerDetails().setCustomerCategory(
				hw_customer.getCustomerCategory() == null ? "" : StringUtils
						.leftPad(hw_customer.getCustomerCategory(), 4, "0"));
		req.getCustomerDetails().setSalutation(
				hw_customer.getSalutation() == null ? "" : StringUtils.leftPad(
						hw_customer.getSalutation(), 4, "0"));
		req.getCustomerDetails().setFirstName(
				hw_customer.getFirstName() == null ? "" : hw_customer
						.getFirstName());
		req.getCustomerDetails().setMiddleName(
				hw_customer.getMiddleName() == null ? "" : hw_customer
						.getMiddleName());
		...
		req.getCustomerDetails()
				.getFamilyContactDetails()
				.setFirstName(
						hw_customer.getFamilyFirstName() == null ? ""
								: hw_customer.getFamilyFirstName());
		req.getCustomerDetails()
				.getFamilyContactDetails()
				.setMiddleName(
						hw_customer.getFamilyMiddleName() == null ? ""
								: hw_customer.getFamilyMiddleName());
		req.getCustomerDetails()
				.getFamilyContactDetails()
				.setLastName(
						hw_customer.getFamilyLastName() == null ? ""
								: hw_customer.getFamilyLastName());
		// 测试数据
		hw_customer.setFamilyRelationship("0001");
		req.getCustomerDetails()
				.getFamilyContactDetails()
				.setRelationship(
						hw_customer.getFamilyRelationship() == null ? ""
								: StringUtils.leftPad(
										hw_customer.getFamilyRelationship(), 4,
										"0"));
		req.getCustomerDetails().setDateOfBirth("");
		if (hw_customer.getDateOfBirth() != null) {
			Date d = DateUtils.formatDate(hw_customer.getDateOfBirth(),
					"dd/MM/yyyy");
			req.getCustomerDetails().setDateOfBirth(
					DateUtils.parse(d.getTime(), "yyyy-MM-dd"));
		}
		req.getCustomerDetails().setGender(
				hw_customer.getGender() == null ? "" : hw_customer.getGender());
		req.getCustomerDetails().setNationality(
				hw_customer.getNationality() == null ? "" : hw_customer
						.getNationality());
		if (hw_customer.getForeignNational() != null) {
			req.getCustomerDetails()
					.setPassportNo(
							hw_customer.getForeignNational().getPassportNo() == null ? ""
									: hw_customer.getForeignNational()
											.getPassportNo());
			req.getCustomerDetails().setVisaNo(
					hw_customer.getForeignNational().getVisaNo() == null ? ""
							: hw_customer.getForeignNational().getVisaNo());

			if (StringUtils.isBlank(hw_customer.getForeignNational()
					.getVisaValidityDate()) == false) {
				Date d = DateUtils.formatDate(hw_customer.getForeignNational()
						.getVisaValidityDate(), "dd/MM/yyyy");
				req.getCustomerDetails().setVisaValidityDate(
						DateUtils.parse(d.getTime(), "yyyy-MM-dd"));
			}
		...
		req.getCustomerDetails()
				.getContactDetails()
				.setMobileNumber(
						hw_customer.getMobileNumber() == null ? ""
								: hw_customer.getMobileNumber());
		req.getCustomerDetails()
				.getContactDetails()
				.setAlternateContactNumberHome(
						hw_customer.getAlternateContactNumberHome() == null ? ""
								: hw_customer.getAlternateContactNumberHome());
		req.getCustomerDetails()
				.getContactDetails()
				.setAlternateContactNumberWork(
						hw_customer.getAlternateContactNumberWork() == null ? ""
								: hw_customer.getAlternateContactNumberWork());
		req.getCustomerDetails()
				.getContactDetails()
				.setEmailId(
						hw_customer.getEmailId() == null ? "" : hw_customer
								.getEmailId());
		...
		if (hw_customer.getPermanentAddress() != null) {
			...
			req.getCustomerDetails()
					.getPermanentAddress()
					.setAddressId(
							address.getAddressId() == null ? "" : address
									.getAddressId());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setBuildingId(
							address.getBuildingId() == null ? "" : address
									.getBuildingId());
			req.getCustomerDetails().getPermanentAddress()
					.setAddressType("PER_ADD");
			req.getCustomerDetails()
					.getPermanentAddress()
					.setCareOf(
							address.getCareOf() == null ? "" : address
									.getCareOf());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setHouseNameORNumber(
							address.getHouseNameORNumber() == null ? ""
									: address.getHouseNameORNumber());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setBuildingNameORNumber(
							address.getBuildingNameORNumber() == null ? ""
									: address.getBuildingNameORNumber());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setSocietyName(
							address.getSocietyName() == null ? "" : address
									.getSocietyName());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setStreetNameORNumber(
							address.getStreetNameORNumber() == null ? ""
									: address.getStreetNameORNumber());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setLandmark(
							address.getLandmark() == null ? "" : address
									.getLandmark());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setSubLocality(
			...
			req.getCustomerDetails()
					.getPermanentAddress()
					.setAreaORTehsil(
							address.getAreaORTehsil() == null ? "" : address
									.getAreaORTehsil());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setPincode(
			...
			req.getCustomerDetails()
					.getPermanentAddress()
					.setVillageORCity(
							address.getVillageORCity() == null ? "" : address
									.getVillageORCity());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setDistrict(
							address.getDistrict() == null ? "" : address
									.getDistrict());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setState(
			...
			req.getCustomerDetails()
					.getPermanentAddress()
					.setCountry(
							address.getCountry() == null ? "" : address
									.getCountry());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setTotalFloors(
							address.getTotalFloors() == null ? "" : address
									.getTotalFloors());
			req.getCustomerDetails()
					.getPermanentAddress()
					.setJioCentreId(
							address.getJioCentreId() == null ? "" : address
									.getJioCentreId());
		}

		req.getCustomerDetails().setPanNumber(
				hw_customer.getPanNumber() == null ? "" : hw_customer
						.getPanNumber());
		req.getCustomerDetails().setPreferredLanguage(
				hw_customer.getPreferredLanguage() == null ? "" : hw_customer
						.getPreferredLanguage());
		req.getCustomerDetails().setPreferredCommunicationChannel(
				hw_customer.getPreferredCommunicationChannel() == null ? ""
						: hw_customer.getPreferredCommunicationChannel());
		req.getCustomerDetails().setAadhaarNumber(
				hw_customer.getAadhaarNumber() == null ? "" : hw_customer
						.getAadhaarNumber());
		req.getCustomerDetails().setMaritalStatus(
				hw_customer.getMaritalStatus() == null ? "" : hw_customer
						.getMaritalStatus());
		req.getCustomerDetails().setAnniversaryDate("2014-06-26");
		if (StringUtils.isBlank(hw_customer.getAnniversaryDate()) == false) {
			Date d = DateUtils.formatDate(hw_customer.getAnniversaryDate(),
					"dd/MM/yyyy");
			req.getCustomerDetails().setAnniversaryDate(
					DateUtils.parse(d.getTime(), "yyyy-MM-dd"));
		}
		req.getCustomerDetails().setOccupation(
				hw_customer.getOccupation() == null ? "" : StringUtils.leftPad(
						hw_customer.getOccupation(), 4, "0"));
		req.getCustomerDetails().setOccupationDescription(
				hw_customer.getOccupationDescription() == null ? ""
						: hw_customer.getOccupationDescription());
		req.getCustomerDetails().setTypeOfHouse(
				hw_customer.getTypeOfHouse() == null ? "" : hw_customer
						.getTypeOfHouse());
		req.getCustomerDetails().setCustomerPictureURL(
				hw_customer.getCustomerPictureURL() == null ? "" : hw_customer
		...
		if (caf.getPayInfo() != null) {
			...
			req.getPaymentDetails().setModeOfPayment(
					payInfo.getModeOfPayment() == null ? "" : payInfo
							.getModeOfPayment());
			req.getPaymentDetails().setPaymentInstrumentNumber(
					payInfo.getPaymentInstrumentNumber() == null ? "" : payInfo
							.getPaymentInstrumentNumber());
			req.getPaymentDetails().setPaymentInstrumentDate("");
			if (StringUtils.isBlank(payInfo.getPaymentInstrumentDate()) == false) {
				Date d = DateUtils.formatDate(
						payInfo.getPaymentInstrumentDate(), "dd/MM/yyyy");
				req.getPaymentDetails().setPaymentInstrumentDate(
						DateUtils.parse(d.getTime(), "yyyy-MM-dd"));
			}
			...
			req.getPaymentDetails().setBankName(
					payInfo.getBankName() == null ? "" : payInfo.getBankName());
			req.getPaymentDetails().setBranchNameAndAddress(
					payInfo.getBranchNameAndAddress() == null ? "" : payInfo
							.getBranchNameAndAddress());
			req.getPaymentDetails().setReceiptNumber(
					payInfo.getReceiptNumber() == null ? "" : payInfo
							.getReceiptNumber());
			req.getPaymentDetails().setCpTransactionId(
					payInfo.getCpTransactionId() == null ? "" : payInfo
							.getCpTransactionId());
		}
		// else
		// {
		...
		if (hw_customer.getPresentAddress() != null) {
			...
			install_address.setAddressId(address.getAddressId() == null ? ""
					: address.getAddressId());
			install_address.setBuildingId(address.getBuildingId() == null ? ""
					: address.getBuildingId());
			install_address.setAddressType("SHIP_TO");
			install_address.setCareOf(address.getCareOf() == null ? ""
					: address.getCareOf());
			install_address
					.setHouseNameORNumber(address.getHouseNameORNumber() == null ? ""
							: address.getHouseNameORNumber());
			install_address.setBuildingNameORNumber(address
					.getBuildingNameORNumber() == null ? "" : address
					.getBuildingNameORNumber());
			install_address
					.setSocietyName(address.getSocietyName() == null ? ""
							: address.getSocietyName());
			install_address.setStreetNameORNumber(address
					.getStreetNameORNumber() == null ? "" : address
					.getStreetNameORNumber());
			install_address.setLandmark(address.getLandmark() == null ? ""
					: address.getLandmark());
			install_address
					.setSubLocality(address.getSubLocality() == null ? ""
							: address.getSubLocality());
			install_address
					.setAreaORTehsil(address.getAreaORTehsil() == null ? ""
							: address.getAreaORTehsil());
			install_address.setPincode(address.getPincode() == null ? ""
					: address.getPincode());
			install_address
					.setVillageORCity(address.getVillageORCity() == null ? ""
							: address.getVillageORCity());
			install_address.setDistrict(address.getDistrict() == null ? ""
					: address.getDistrict());
			install_address.setState(address.getState() == null ? "" : address
					.getState());
			install_address.setCountry(address.getCountry() == null ? ""
					: address.getCountry());
			install_address
					.setTotalFloors(address.getTotalFloors() == null ? ""
							: address.getTotalFloors());
			install_address
					.setJioCentreId(address.getJioCentreId() == null ? ""
							: address.getJioCentreId());
		...
		req.getCafDetails().setMerchantCode(
				caf.getMerchantCode() == null ? "" : caf.getMerchantCode());
		req.getCafDetails().setPosAgentCode(
				caf.getPosAgentCode() == null ? "" : caf.getPosAgentCode());
		req.getCafDetails().setPosAgentSignatureDate("");
		if (StringUtils.isBlank(caf.getPosAgentSignatureDate()) == false) {
			Date d = DateUtils.formatDate(caf.getPosAgentSignatureDate(),
					"dd/MM/yyyy");
			req.getCafDetails().setPosAgentSignatureDate(
					DateUtils.parse(d.getTime(), "yyyy-MM-dd"));
		}
		...
		req.getCafDetails().setCustomerDeclarationPlace(
				caf.getCustomerDeclarationPlace() == null ? "" : caf
						.getCustomerDeclarationPlace());
		req.getCafDetails().setCustomerDeclarationDate("");
		if (StringUtils.isBlank(caf.getCustomerDeclarationDate()) == false) {
			Date d = DateUtils.formatDate(caf.getCustomerDeclarationDate(),
					"dd/MM/yyyy");
			req.getCafDetails().setPosAgentSignatureDate(
					DateUtils.parse(d.getTime(), "yyyy-MM-dd"));
		}
		...
		if (caf.getPoa() != null) {
			...
			poa.setIdProofType(caf.getPoa().getIdProofType() == null ? "" : caf
					.getPoa().getIdProofType());
			poa.setDocumentNumber(caf.getPoa().getDocumentNumber() == null ? ""
					: caf.getPoa().getDocumentNumber());
			poa.setDateOfIssue("");
			if (caf.getPoi().getDateOfIssue() != null) {
				Date d = DateUtils.formatDate(caf.getPoi().getDateOfIssue(),
						"dd/MM/yyyy");
			...
			poa.setPlaceOfIssue(caf.getPoa().getPlaceOfIssue() == null ? ""
					: caf.getPoa().getPlaceOfIssue());
			poa.setIssuingAuthority(caf.getPoa().getIssuingAuthority() == null ? ""
					: caf.getPoa().getIssuingAuthority());
			poa.setIdentifierURL(caf.getPoa().getIdentifierURL() == null ? ""
					: caf.getPoa().getIdentifierURL());
			poa.setAadhaarTransactionRefNo(caf.getPoa()
					.getAadhaarTransactionRefNo() == null ? "" : caf.getPoa()
					.getAadhaarTransactionRefNo());
		...
		if (caf.getPoi() != null) {
			...
			poi.setIdProofType(caf.getPoi().getIdProofType() == null ? "" : caf
					.getPoi().getIdProofType());
			poi.setDocumentNumber(caf.getPoi().getDocumentNumber() == null ? ""
					: caf.getPoi().getDocumentNumber());
			poi.setDateOfIssue("");
			if (caf.getPoi().getDateOfIssue() != null) {
				Date d = DateUtils.formatDate(caf.getPoi().getDateOfIssue(),
						"dd/MM/yyyy");
			...
			poi.setPlaceOfIssue(caf.getPoi().getPlaceOfIssue() == null ? ""
					: caf.getPoi().getPlaceOfIssue());
			poi.setIssuingAuthority(caf.getPoi().getIssuingAuthority() == null ? ""
					: caf.getPoi().getIssuingAuthority());
			poi.setIdentifierURL(caf.getPoi().getIdentifierURL() == null ? ""
					: caf.getPoi().getIdentifierURL());
			poi.setAadhaarTransactionRefNo(caf.getPoi()
					.getAadhaarTransactionRefNo() == null ? "" : caf.getPoi()
					.getAadhaarTransactionRefNo());
			req.getCafDetails().getProofs().add(poi);
		}

		req.getCafDetails().setCurrentMobileConnections(
				new ArrayList<YD0010Request.Connection>(0));
		// if(caf.getOperators() != null && caf.getOperators().isEmpty() ==
		// false)
		// {
		...
		if (caf.getLocalReferenceDetails() != null) {
			...
			req.getCafDetails()
					.getLocalReferenceDetails()
					.setFirstName(
							lr.getFirstName() == null ? "" : lr.getFirstName());
			req.getCafDetails()
					.getLocalReferenceDetails()
					.setMiddleName(
							lr.getMiddleName() == null ? "" : lr
									.getMiddleName());
			req.getCafDetails()
					.getLocalReferenceDetails()
					.setLastName(
							lr.getLastName() == null ? "" : lr.getLastName());
			req.getCafDetails()
					.getLocalReferenceDetails()
					.setContactNumber(
							lr.getContactNumber() == null ? "" : lr
									.getContactNumber());
			req.getCafDetails().getLocalReferenceDetails()
					.seteMailId(lr.geteMailId() == null ? "" : lr.geteMailId());
			...
			if (lr.getAddress() != null) {
				...
				req.getCafDetails().getLocalReferenceDetails()
						.setAddress(yd_lr_address);
				yd_lr_address
						.setAddressId(hw_lr_address.getAddressId() == null ? ""
								: hw_lr_address.getAddressId());
				yd_lr_address
						.setBuildingId(hw_lr_address.getBuildingId() == null ? ""
								: hw_lr_address.getBuildingId());
				yd_lr_address.setAddressType("REF_ADD");
				yd_lr_address.setCareOf(hw_lr_address.getCareOf() == null ? ""
						: hw_lr_address.getCareOf());
				yd_lr_address.setHouseNameORNumber(hw_lr_address
						.getHouseNameORNumber() == null ? "" : hw_lr_address
						.getHouseNameORNumber());
				yd_lr_address.setBuildingNameORNumber(hw_lr_address
						.getBuildingNameORNumber() == null ? "" : hw_lr_address
						.getBuildingNameORNumber());
				yd_lr_address
						.setSocietyName(hw_lr_address.getSocietyName() == null ? ""
								: hw_lr_address.getSocietyName());
				yd_lr_address.setStreetNameORNumber(hw_lr_address
						.getStreetNameORNumber() == null ? "" : hw_lr_address
						.getStreetNameORNumber());
				yd_lr_address
						.setLandmark(hw_lr_address.getLandmark() == null ? ""
								: hw_lr_address.getLandmark());
				yd_lr_address
						.setSubLocality(hw_lr_address.getSubLocality() == null ? ""
								: hw_lr_address.getSubLocality());
				yd_lr_address
						.setAreaORTehsil(hw_lr_address.getAreaORTehsil() == null ? ""
								: hw_lr_address.getAreaORTehsil());
				yd_lr_address
						.setPincode(hw_lr_address.getPincode() == null ? ""
								: hw_lr_address.getPincode());
				yd_lr_address
						.setVillageORCity(hw_lr_address.getVillageORCity() == null ? ""
								: hw_lr_address.getVillageORCity());
				yd_lr_address
						.setDistrict(hw_lr_address.getDistrict() == null ? ""
								: hw_lr_address.getDistrict());
				yd_lr_address.setState(hw_lr_address.getState() == null ? ""
						: hw_lr_address.getState());
				yd_lr_address
						.setCountry(hw_lr_address.getCountry() == null ? ""
								: hw_lr_address.getCountry());
				yd_lr_address
						.setTotalFloors(hw_lr_address.getTotalFloors() == null ? ""
								: hw_lr_address.getTotalFloors());
				yd_lr_address
						.setJioCentreId(hw_lr_address.getJioCentreId() == null ? ""
								: hw_lr_address.getJioCentreId());
			}
		}

		/**** ReferringCustomer ****/
		req.getCafDetails()
				.setReferringCustomerDetails(new ReferringCustomer());
		req.getCafDetails()
				.getReferringCustomerDetails()
				.setId(caf.getReferringCustomerId() == null ? "" : caf
						.getReferringCustomerId());
		req.getCafDetails()
				.getReferringCustomerDetails()
				.setReferenceMobileNumber(
						caf.getReferenceMobileNumber() == null ? "" : caf
								.getReferenceMobileNumber());

		/************************* LocalReference *******************************/
		if (caf.getLocalReferenceDetails() != null) {
			req.getCafDetails().setLocalReferenceVerification(
					new LocalRefVerify());
			req.getCafDetails()
					.getLocalReferenceVerification()
					.setCallingPartyNumber(
							caf.getLocalReferenceDetails()
									.getCallingPartyNumber() == null ? "" : caf
									.getLocalReferenceDetails()
		...
		if (hw_caf_order != null) {
			...
			order.setAccountId(hw_order.getAccountId() == null ? "" : hw_order
					.getAccountId());
			order.setOfferId(hw_order.getOfferId() == null ? "" : hw_order
					.getOfferId());
			order.seteWalletReservationReferenceId(hw_order
					.geteWalletReservationReferenceId() == null ? "" : hw_order
					.geteWalletReservationReferenceId());
			...
			if (StringUtils.isBlank(hw_caf_order.getOfferId()) == false) {
				...
				order.setProducts(new ArrayList<YD0010Request.Product>());
			
				if (productSpecMapping != null&& productSpecMapping.getProductSpecs() != null&& productSpecMapping.getProductSpecs().isEmpty() == false) {
					List<YD0010Request.ProductIdentifier> listProductIdentifier  = new ArrayList<YD0010Request.ProductIdentifier>(0);
					for (ProductSpecMapping.ProductSpec productSpec : productSpecMapping.getProductSpecs()) {
						...
						//xuzhou start
						List<com.ailk.yd.mapp.client.model.HW0010Request.Identifier> identifierList= identifierMap.get("1").get(productSpec.getProductSpecificationId());
					   System.out.println(identifierList.size());
						
						if(identifierList!=null&&identifierList.isEmpty()==false){
							for(int i=0;i<identifierList.size();i++){
								HW0010Request.Identifier  itf=identifierList.get(i); 
								YD0010Request.ProductIdentifier  productIdentifiers= new YD0010Request.ProductIdentifier();
								productIdentifiers.setComponentPrice(new IdObject(itf.getComponentPriceId()));
								productIdentifiers.setValue(itf.getValue());
								productIdentifiers.setName(itf.getName());
								productIdentifiers.setType(itf.getName());
								listProductIdentifier.add(productIdentifiers);
							}
						}
						//xuzhou end
						ps.setIdentifier(listProductIdentifier);
						...
						List<YD0010Request.Device> deviceList = 	new ArrayList<YD0010Request.Device>(0);
						...
						for (ProductSpecMapping.ResourceSpec resourceSpec : productSpec.getResourceSpecList()) {
							...
							if(identifierLists!=null&&identifierLists.isEmpty()==false){
								for(int i=0;i<identifierLists.size();i++){
									HW0010Request.Identifier  itf=identifierLists.get(i); 
									YD0010Request.NameAndValueObject  naAndValue= new YD0010Request.NameAndValueObject();
									naAndValue.setValue(itf.getValue());
									naAndValue.setName(itf.getName());
									listNavoIdentifier.add(naAndValue);
								}
							}
							//xuzhou end
							device.setIdentifier(listNavoIdentifier);
	...
	private void validate(YD0010Request request) throws Exception {
		...
		if (StringUtils.isEmpty(customer.getPanNumber())
				&& caf.getForm61Details() == null)
			throw new Exception("PanNumber is null,form61 to be filled");
		// Outstation 类型为 5
		if ("5".equals(customer.getCustomerCategory())
				&& caf.getLocalReferenceDetails() == null) {
			throw new Exception(
					"Outstation Customer to be filled Local Reference info");
		}

		// visa号码不为空，而visa有效期为空
		if (StringUtils.isBlank(customer.getVisaNo()) == false
				&& StringUtils.isBlank(customer.getVisaValidityDate())) {
